 ...
 * is that theme_amelia does not define any layout files to re-use the
...
if ($ADMIN->fulltree) {
    ...
    // Logo file setting.
    $name = 'theme_amelia/logo';
    $title = get_string('logo','theme_amelia');
    $description = get_string('logodesc', 'theme_amelia');
    $setting = new admin_setting_configstoredfile($name, $title, $description, 'logo');
    $setting->set_updatedcallback('theme_reset_all_caches');
    $settings->add($setting);

    // Custom CSS file.
    $name = 'theme_amelia/customcss';
    $title = get_string('customcss', 'theme_amelia');
    $description = get_string('customcssdesc', 'theme_amelia');
    $default = '';
